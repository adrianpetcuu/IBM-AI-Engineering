...
# Identify indices where the feature variable X is greater that a certain threshold
...
plt.scatter(X, y_ideal, linewidth=4, color='g', label='Ideal, noise free data')
plt.xlabel('Feature (X)')
plt.ylabel('Target (y)')
plt.title('')
plt.legend()
...
# Plot prediction from the ridge regression model
plt.plot(X, y_outlier_pred_ridge, linestyle='--', linewidth=2, label='Ridge Regression')
# Plot prediction from the lasso regression model
...
# Fit a lasso regression model (reqularization to control large coefficients)
...
# Plot the ideal regression line (noise free data)
plt.plot(X, y_ideal, linewidth=2, color='k', label='Ideal, noise free data')
# Plot prediction from the simple linear regression model
...
plt.ylabel('Target (y)')
# plt.ylim((0, 20))
plt.title('Comparison of prediction with no outliers')
plt.legend()
plt.show()
...
# the three different linear regression models again on th resulting data
...
# Print the regression preformance results
regression_results(y_test, y_pred_linear, 'Ordinary')
regression_results(y_test, y_pred_ridge, 'Ridge')
regression_results(y_test, y_pred_lasso, 'Lasso')
